...
public class ForwardGoal : AIGoal
{
    ...
    public override float Plan()
    {
        ...
        var targetDirection = 0.5f * (Vector2.right + ProjectedShadow.normalized);
        ...
        // right vector.That is, when it is most efficient to speed up. This also helps
        // with stability as the goal will prioritise speed less when on tricky terrain.
        return Mathf.Clamp01(Vector2.Dot(Velocity.normalized, Vector2.right));
    }
